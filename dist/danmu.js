...
var Danmu = (function () {
    function Danmu(roomId) {
        this.CIDInfoUrl = "http://live.bilibili.com/api/player?id=cid:";
        this.chatPort = 788;
        this.protocolVersion = 1;
        this.connected = false;
        this.chatHost = "livecmt-1.bilibili.com";
        this.uid = Math.floor(100000000000000.0 + 200000000000000.0 * Math.random());
        this.http = axios_1.default.create({
            timeout: 2000,
        });
        this.okFlag = Buffer.from("00000010001000010000000800000001", "hex");
        this.cache = null;
        this.roomId = roomId;
    }
                ...
                switch (_a.label) {
                    ...
                    case 2:
                        ...
                        this.client.on("data", function (data) {
                            ...
                            if (_this.connected) {
                                if (_this.cache) {
                                    var msg_1 = helper.unpacket(Buffer.concat([_this.cache, data]));
                                    _this.cache = null;
                                    return helper.log(msg_1.data);
                                }
                                if (data.byteLength === 1452) {
                                    _this.cache = data;
                                    return;
                                }
                                var msg = helper.unpacket(data);
